...
#define ips114_write_8bit_data(data)    (SPI_Send8BitData(IPS114_SPI, (data)))
#define ips114_write_16bit_data(data)   (SPI_Send16BitData(IPS114_SPI, (data)))
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
static void ips114_write_index (const uint8_t dat)
{
    IPS114_DC(0);
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
static void ips114_set_region (const uint16_t x1, const uint16_t y1, const uint16_t x2, const uint16_t y2)
{
...
// 返回参数     void
// 使用示例     ips114_show_int(0, 0, x, 3);                    // x 可以为 int32_t int16_t int8_t 类型
...
// 参数说明     num             需要显示的位数 最高10位  不包含正负号
// 返回参数     void
// 使用示例     ips114_show_uint(0, 0, x, 3);                   // x 可以为 uint32_t uint16_t uint8_t 类型
// 备注信息     负数会显示一个 ‘-’号   正数显示一个空格
//-------------------------------------------------------------------------------------------------------------------
...
{
    SPI_Init(SPI_MODE0, SPI_CS_MODE_NULL, IPS114_SPI_SPEED, IPS114_SCL_PIN, IPS114_SDA_PIN, SPI_MISO_NULL, SPI_CS_NULL);

    GPIO_setAsOutputPin(IPS114_DC_PORT, IPS114_DC_PIN);
    GPIO_setAsOutputPin(IPS114_RST_PORT, IPS114_RST_PIN);
    GPIO_setAsOutputPin(IPS114_CS_PORT, IPS114_CS_PIN);
    GPIO_setAsOutputPin(IPS114_BLK_PORT, IPS114_BLK_PIN);
