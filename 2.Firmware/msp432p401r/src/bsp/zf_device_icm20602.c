...
static SoftIIC_t icm20602_iic_struct;
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define icm20602_write_register(reg, data)      (SoftIIC_Write8BitRegister(&icm20602_iic_struct, (reg), (data)))
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define icm20602_read_register(reg)             (SoftIIC_Read8BitRegister(&icm20602_iic_struct, (reg)))
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define icm20602_read_registers(reg, data, len) (SoftIIC_Read8BitRegisterArray(&icm20602_iic_struct, (reg), (data), (len)))
#else
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     ICM20602 写寄存器
// 参数说明     reg             寄存器地址
...
{
    ICM20602_CS(0);
    SPI_Write8BitRegister(ICM20602_SPI, reg | ICM20602_SPI_W, data);
    ICM20602_CS(1);
}
...
static uint8_t icm20602_read_register(uint8_t reg)
{
    uint8_t dat;
    ICM20602_CS(0);
    dat = SPI_Read8BitRegister(ICM20602_SPI, reg|ICM20602_SPI_R);
    ICM20602_CS(1);
    return dat;
}
...
{
    ICM20602_CS(0);
    SPI_Read8BitRegisterArray(ICM20602_SPI, reg|ICM20602_SPI_R, dat, len);
    ICM20602_CS(1);
}
#endif
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float icm20602_acc_transition (int16_t acc_value)
{
    float acc_data = 0;
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float icm20602_gyro_transition (int16_t gyro_value)
{
    float gyro_data = 0;
