...
#define SDA GPIO_getInputPinValue(GPIO_PORT_P3,GPIO_PIN6)
#define SDA0() GPIO_setOutputLowOnPin(GPIO_PORT_P3,GPIO_PIN6)
#define SDA1() GPIO_setOutputHighOnPin(GPIO_PORT_P3,GPIO_PIN6)
#define SCL0() GPIO_setOutputLowOnPin(GPIO_PORT_P3,GPIO_PIN7)
#define SCL1() GPIO_setOutputHighOnPin(GPIO_PORT_P3,GPIO_PIN7)
#define DIR_OUT() GPIO_setAsOutputPin(GPIO_PORT_P3,GPIO_PIN6)
#define DIR_IN()  GPIO_setAsInputPin(GPIO_PORT_P3,GPIO_PIN6)               
...
uint16_t simiic_delay_time = SIMIIC_DELAY_TIME; // ICM等传感器应设置为20
...
void simiic_write_regs(uint8_t dev_add, uint8_t reg, uint8_t *dat_add, uint8_t num)
{
    ...
    {
        simiic_write_reg(dev_add, reg + i, dat_add[i]);
    }
}
...
void simiic_read_regs(uint8_t dev_add, uint8_t reg, uint8_t *dat_add, uint8_t num, IIC_type type)
{
    ...
    else
    {
        for (uint8_t i = 0; i < num; i++)
        {
            dat_add[i] = simiic_read_reg(dev_add, reg + i, type);
        }
    }
}
