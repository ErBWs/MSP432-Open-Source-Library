...
static SoftIIC_t imu963ra_iic_struct;
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define imu963ra_write_acc_gyro_register(reg,data)       (SoftIIC_Write8BitRegister(&imu963ra_iic_struct,reg,data))
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define imu963ra_read_acc_gyro_register(reg)             (SoftIIC_SCCB_ReadRegister(&imu963ra_iic_struct,reg))
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
#define imu963ra_read_acc_gyro_registers(reg,data,len)   (SoftIIC_Read8BitRegisterArray(&imu963ra_iic_struct,reg,data,len))
#else
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IMU963RA 写寄存器
// 参数说明     reg             寄存器地址
...
{
    IMU963RA_CS(0);
    SPI_Write8BitRegister(IMU963RA_SPI, reg | IMU963RA_SPI_W, data);
...
static uint8_t imu963ra_read_acc_gyro_register(uint8_t reg)
{
    uint8_t data = 0;
    IMU963RA_CS(0);
    data = SPI_Read8BitRegister(IMU963RA_SPI, reg | IMU963RA_SPI_R);
...
{
    IMU963RA_CS(0);
    SPI_Read8BitRegisterArray(IMU963RA_SPI, reg | IMU963RA_SPI_R, data, len);
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float imu963ra_acc_transition (int16_t acc_value)
{
    float acc_data = 0;
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float imu963ra_gyro_transition (int16_t gyro_value)
{
    float gyro_data = 0;
...
// 返回参数     void
// 使用示例     float data = imu963ra_mag_transition(imu963ra_mag_x);          // 单位为G(高斯)
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float imu963ra_mag_transition (int16_t mag_value)
{
    float mag_data = 0;
...
#if IMU963RA_USE_SOFT_IIC
    SoftIIC_Init(&imu963ra_iic_struct, IMU963RA_DEV_ADDR, IMU963RA_SOFT_IIC_DELAY, IMU963RA_SCL_PORT, IMU963RA_SCL_PIN, IMU963RA_SDA_PORT, IMU963RA_SDA_PIN);
#else
    SPI_Init(SPI_MODE0, SPI_CS_MODE_NULL, IMU963RA_SPI_SPEED, IMU963RA_SPC_PIN, IMU963RA_SDI_PIN, IMU963RA_SDO_PIN, SPI_CS_NULL);
    GPIO_setAsOutputPin(IMU963RA_CS_PORT, IMU963RA_CS_PIN);
#endif
